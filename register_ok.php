...
<?php
$rs = "SELECT * FROM tbregister ORDER BY id desc";
...
<table class="display normal-t" cellspacing="0" cellpadding="0" border="0">
    ...
    <tr>
        <td>หลักสูตรที่เรียน :</td>
        <td>
            <?php
            $rs_c = mysql_query('SELECT * FROM tbcourse WHERE id = ' . $row['course_id']);
            while ($val_c = mysql_fetch_assoc($rs_c)):
                ?>

            <?php echo $val_c['c_name']; ?>  ;	
           <?php echo $val_c['c_price']; ?> บาท
        <?php endwhile; ?>
        </td>
    </tr>
    <tr>
